	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider, TaskProviderLike
{
	...
	private static activeInstances: Set<SheetsProvider> = new Set()
	...
	constructor(
		...
		this.log("SheetsProvider instantiated")
		SheetsProvider.activeInstances.add(this)
	...
	*/
	...
	async dispose() {
		this.log("Disposing SheetsProvider...")
		...
		SheetsProvider.activeInstances.delete(this)
	...
	public static getVisibleInstance(): SheetsProvider | undefined {
		return findLast(Array.from(this.activeInstances), (instance) => instance.view?.visible === true)
	}

	public static async getInstance(): Promise<SheetsProvider | undefined> {
		let visibleProvider = SheetsProvider.getVisibleInstance()
		...
		if (!visibleProvider) {
			...
			await delay(100)
			visibleProvider = SheetsProvider.getVisibleInstance()
		}
	...
	public static async isActiveTask(): Promise<boolean> {
		const visibleProvider = await SheetsProvider.getInstance()
	...
	async resolveWebviewView(webviewView: vscode.WebviewView | vscode.WebviewPanel) {
		...
		webviewView.onDidDispose(
			async () => {
				if (inTabMode) {
					this.log("Disposing SheetsProvider instance for tab view")
					await this.dispose()
				} else {
	...
	private async getHMRHtmlContent(webview: vscode.Webview): Promise<string> {
		...
		try {
			...
			if (fs.existsSync(portFilePath)) {
				...
				console.log(`[SheetsProvider:Vite] Using Vite server port from ${portFilePath}: ${localPort}`)
			} else {
				console.log(
					`[SheetsProvider:Vite] Port file not found at ${portFilePath}, using default port: ${localPort}`,
				)
			}
		} catch (err) {
			console.error("[SheetsProvider:Vite] Failed to read Vite port file:", err)
