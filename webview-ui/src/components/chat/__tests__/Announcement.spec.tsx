	...
	useAppTranslation: () => ({
		t: (key: string, options?: { version: string }) => {
			if (key === "chat:announcement.title") {
				return `🎉 Sheets IDE ${options?.version} Released`
			}
			if (key === "chat:announcement.description") {
				return `Sheets IDE ${options?.version} brings powerful new features and improvements based on your feedback.`
			}
...
describe("Announcement", () => {
	...
	it("renders the announcement with the version number from package.json", () => {
		...
		expect(screen.getByText(`🎉 Sheets IDE ${expectedVersion} Released`)).toBeInTheDocument()
		expect(
			screen.getByText(
				`Sheets IDE ${expectedVersion} brings powerful new features and improvements based on your feedback.`,
			),
		).toBeInTheDocument()
	})
})
